...
>;
type Conntrack<W, A, P> = Arc<RwLock<conntrack::Streams<WriteStream<W, A, P>>>>;
    ...
    Stream::Write: HasStableId,
{
    ...
    streams: Conntrack<Stream::Write, Addr, Broadcast>,
    membership: Arc<RwLock<membership::Hpv<Pcg64Mcg, Stream::Id, Addr>>>,
    ...
    Stream::Id: Clone + Debug + Ord + Send + Sync + 'static,
{
    pub fn new(
        ...
        let streams = Arc::new(RwLock::new(Default::default()));
        let membership = Arc::new(RwLock::new(
            membership::Hpv::new(local_id, prng).with_params(mparams),
        ));
    ...
    fn has_active(&self) -> bool {
        self.membership.read().unwrap().num_active() > 0
    }
    ...
    pub(super) async fn outgoing_bidi(
        ...
        let tick = {
            ...
            membership.connection_established(
                PartialPeerInfo {
    ...
    pub(super) async fn incoming_bidi(
...

        {
            ...
            let _prev = streams.insert(send.clone());
        ...
        let tick = if was_removed {
            ...
            let mut membership = self.membership.write().unwrap();
            membership.connection_lost(remote_id, stream_id)
        } else {
            None
        };
        if let Some(tock) = tick {
    ...
    async fn handle_membership(
        ...
        let tick = {
            let mut membership = self.membership.write().unwrap();
            membership.apply(remote_id, remote_addr, stream_id, msg)
        };
        let tick = tick.map_err(|e| {
        ...
        let streams = {
            ...
            streams
                ...
                .filter_map(|(peer_id, stream)| {
                    recipients.get(peer_id).map(|dest_stream_id| {
                        debug_assert!(*dest_stream_id == stream.stable_id());
                        stream.clone()
                    })
                })
                .collect::<Vec<_>>()
    ...
    async fn on_send_error(&self, peer: PeerId, stream: Stream::Id, err: CborCodecError) {
        ...
        future::join(
            ...
            async {
                let cont_tick = {
                    let mut membership = self.membership.write().unwrap();
                    membership.connection_lost(peer, stream)
                };
    ...
    async fn broadcast(
        ...
        let recipients = {
            ...
            membership.broadcast_recipients(exclude).collect::<Vec<_>>()
        };
    ...
    async fn promote_random(&self) {
        ...
        let candidates = {
            let mut membership = self.membership.write().unwrap();
            membership.choose_passive_to_promote()
        };
        match candidates {
